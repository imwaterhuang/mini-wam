...
Mini-WAM 是一个面向机器人算法与具身智能岗位的端到端工程项目：在 Push-T 仿真任务中，完成示范数据处理、动作块策略训练、训练恢复、配对闭环评测与失败分析，并检验训练期的动作条件未来表示监督是否能改善控制表现。

WAM（World Action Model，世界动作模型）是项目名称；这里预测的是未来视觉表示，而不是生成未来视频。推理阶段只运行策略网络，不进行世界模型规划。
...
当前回归记录为 57 项常规测试通过，另有 4 项默认跳过的多进程测试单独验证通过，共 61 项。

## 代码与证据入口

- [环境与数据安装](docs/GETTING_STARTED.md)
- [训练与断点恢复](COLAB.md)
- [评测和复现命令](docs/REPRODUCING.md)
