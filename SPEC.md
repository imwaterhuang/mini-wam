...
- 最终结论以冻结的随机 Push-T 测试初始状态上的闭环指标为主，离线验证指标只用于诊断。
...
- 使用两层 MLP 输出 `[B, 4, 512]`，最后一维做 L2 归一化。
...
- 每 5000 步在固定 20 个开发评估初始状态上运行闭环评测。
- 以开发评估集平均覆盖率选择最佳 checkpoint。
- 覆盖率接近时，以示范数据验证集动作损失较低者为准。
- 最终闭环测试只使用该规则选中的单个 checkpoint。
...
负结果仍然有效，但必须说明失败位于数据、优化、动作敏感性、表征冲突、闭环分布偏移还是评测方差。
